...
export type SpellPdfRequestBody = {
  spellIds?: string[];
  paperSize?: "letter" | "a4";
  marginInches?: number;
...
export type SpellPdfRequestPayload = {
  spells: Spell[];
  paperSize: "letter" | "a4";
  marginInches: number;
  showCropMarks: boolean;
...
export async function loadSpellPdfPayload(
  req: Request
): Promise<{ payload: SpellPdfRequestPayload } | { error: NextResponse }> {
  ...
  let body: SpellPdfRequestBody;
  try {
    body = (await req.json()) as SpellPdfRequestBody;
  } catch {
  ...
  if (spellIds.length === 0) {
    return { error: NextResponse.json({ error: "spellIds required" }, { status: 400 }) };
  }
  ...
  const spellsDb = await db.spell.findMany({
    where: {
      OR: [{ id: { in: uniqueKeys } }, { title: { in: uniqueKeys } }],
    },
  });
  ...
  for (const key of spellIds) {
    ...
    spells.push(serializeSpellForPdf(row));
  }
